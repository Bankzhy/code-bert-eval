...
class BartRobertaClassificationHead(nn.Module):
...
class BartCloneModel(nn.Module):
    def __init__(self, encoder, config, code_vocab, args):
        super(BartCloneModel, self).__init__()
        self.encoder = encoder
        self.config = config
        # self.tokenizer = tokenizer
        # self.classifier = BartRobertaClassificationHead(config)
        self.classifier = nn.Linear(config.hidden_size, 2)
        self.code_vocab = code_vocab
        self.args = args

    def get_bart_vec(self, input_ids, attention_mask, decoder_input_ids, decoder_attention_mask):
        ...
        code_hidden_states = self.encoder(
            input_ids=input_ids,
        ...
        code_eos_mask = input_ids.eq(self.code_vocab.get_eos_index())
    ...
    def forward(self, input_ids, attention_mask, decoder_input_ids, decoder_attention_mask, labels=None):
        ...
        vec = self.get_bart_vec(input_ids, attention_mask, decoder_input_ids, decoder_attention_mask)
